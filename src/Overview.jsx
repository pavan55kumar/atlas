...
const QuickAction = memo(function QuickAction({ icon, label, color, onClick, isHoverable, variant }) {
  return (
    <motion.button
      onClick={onClick}
      whileHover={isHoverable ? { y: -3, scale: 1.03 } : undefined}
      whileTap={{ scale: 0.94 }}
      transition={springTap}
      className={'ov-quick-action' + (variant === 'hero' ? ' ov-quick-action--hero' : '')}
      style={{ '--qa-color': color }}
    >
      <span className="ov-quick-action-icon">{icon}</span>
      <span className="ov-quick-action-label">{label}</span>
    </motion.button>
  )
})
...
function Overview({ userId, onNavigate }) {
  ...
  return (
    <div className="ov-page">
      ...
      {/* 1. Hero — Command Center with integrated Quick Actions */}
      <motion.div
        {...heroFade}
      ...
      >
        ...
        <div className="ov-hero-inner">
          ...
          <div className="ov-hero-actions">
            <QuickAction variant="hero" icon={quickTaskIcon} label="Task" color="#6C6CF0" onClick={goToTasks} isHoverable={isHoverable} />
            <QuickAction variant="hero" icon={quickHabitIcon} label="Habit" color="#F0876C" onClick={goToHabits} isHoverable={isHoverable} />
            <QuickAction variant="hero" icon={quickGoalIcon} label="Goal" color="#6CC7F0" onClick={goToGoals} isHoverable={isHoverable} />
            <QuickAction variant="hero" icon={quickEventIcon} label="Event" color="#8CF06C" onClick={goToCalendar} isHoverable={isHoverable} />
          </div>
        </div>
      </motion.div>
